...
public class JwtService {
     ...
     * Reconstruye la clave a partir de la cadena codificada en Base64 definida en el archivo de configuración (application.properties).
     * asegurando el uso del token despues del reinicio de la app.
     * @return
     */
    ...
    public String generateToken(UserDetails userDetails) {
        ...
        return Jwts.builder()
                .claims(claims) // permisos que tiene el usuario
                .subject(userDetails.getUsername()) // username
                .issuedAt(new Date(System.currentTimeMillis())) // hora en la que se emite el token
                //.expiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60)) // 1 hora
                .expiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * Integer.parseInt(expiration))) // expiracion del token (1 hora)
                .signWith(getSigningKey(), Jwts.SIG.HS256) // firma del token
